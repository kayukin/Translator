...
namespace Dictionary
{
	class Locator
	{
		...
		std::shared_ptr<ITranslatorController> translator_controller;
	public:
		...
		std::shared_ptr<ITranslatorController> getTranslatorController(){ return translator_controller; }
		...
		std::shared_ptr<ITranslator> getTranslator(std::shared_ptr<IDictionary>& first, std::shared_ptr<IDictionary>& second, TranslationState state){ return std::shared_ptr<ITranslator>(new Translator(first, second, state, lang_detector, translator_controller)); }
	private:
		Locator()
		{
			...
			translator_controller = std::shared_ptr<ITranslatorController>(new TranslatorController(settings_loader));
		}
		~Locator(){}
